...
// Message Preview View without scale effect
struct MessagePreviewView: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 4)
        .fixedSize(horizontal: true, vertical: true) // Позволяет view принять размер содержимого
    }
...
#Preview {
    MessagePreviewView(message: Message(text: "Hello!", isFromCurrentUser: true, timestamp: Date()))
}
